...
int al_clear(ArrayList* pList)
{
    ...
    if(pList!=NULL)
    {
        if(pList->reservedSize>AL_INITIAL_VALUE)
        {
            pElements=malloc(sizeof(void *)*AL_INITIAL_VALUE );
            if(pElements!=NULL)
            {
...
ArrayList* al_subList(ArrayList* pList,int from,int to)
{
    ArrayList* returnAux = NULL;
    int i;
    if(pList!=NULL&&from>=0&&to<=pList->size&&(from<to||from==to))
    {
        returnAux=al_newArrayList();
        if(from==to)
        {
            returnAux->add(returnAux,pList->get(pList,from));
        }
        else
        {
            for(i=from;i<to;i++)
...
int al_containsAll(ArrayList* pList,ArrayList* pList2)
{
    ...
    if(pList!=NULL&&pList2!=NULL)
    {
        ...
        {
            if((al_contains(pList,pList2->get(pList2,i))==0))
            {
                returnAux=0;
                break;
            }
...
int al_sort(ArrayList* pList, int (*pFunc)(void* ,void*), int order)
{
    ...
    if(pList!=NULL&&pFunc!=NULL&&(order==1||order==0))
    {
        for(i=0;i<pList->size-1;i++)
        {
            ...
            {
                if(order==0&&pFunc(pList->get(pList,i),pList->get(pList,j))==-1)
                {
                    pAuxElement=pList->get(pList,i);
                    pList->set(pList,i,pList->get(pList,j));
                    pList->set(pList,j,pAuxElement);
                }
                else if(order==1&&pFunc(pList->get(pList,i),pList->get(pList,j))==1)
                {
                    pAuxElement=pList->get(pList,i);
                    pList->set(pList,i,pList->get(pList,j));
                    pList->set(pList,j,pAuxElement);
...
int resizeUp(ArrayList* pList)
{
    ...
    if(pList!=NULL)
    {
        ...
        {
            auxP=realloc(pList->pElements,(sizeof(void*)*(AL_INCREMENT+pList->size)));
            if(auxP!=NULL)
            {
                pList->pElements=auxP;
